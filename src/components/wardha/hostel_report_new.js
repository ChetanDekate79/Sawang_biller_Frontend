...
const Hostel_report_new = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE_URL}/hostel_report?client_id=${selectedHost}&date=${selectedDate}`)
  ...
  const handleDownloadPDF = () => {
    ...
    html2canvas(content, { scale: 2, useCORS: true }).then((canvas) => {
      ...
      pdf.save(`${selectedHost}-${selectedDate}.pdf`);
    });
  };
  ...
  const handleDownloadCSV = () => {
    ...
    a.href = url;
    a.download = `${selectedHost}-${selectedDate}.csv`;
...
{isLoading ? (
      ...
      <div className="container-fluid mt-3" style={{fontFamily: "Comic Sans MS"}}>
        <h2 className="text-center" >{selectedHost} Hostel Report - {selectedDate}</h2>
...
<tfoot className="table-secondary" style={{ fontWeight: 'bold' }}>
  <tr>
    ...
    <td>{data[1].sum_ryb}</td>
    <td>{data[1].sum_common_area}</td>
    <td>{data[1].sum_ca_per}</td>
  </tr>
</tfoot>
...
export default Hostel_report_new;